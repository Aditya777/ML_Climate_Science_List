...
from constants import ifile, weights_file
...
# ifile = 'puma_sample_30year_normalized.nc'
...
N_train_usedintuning = 20 * 365  # 30 * 365
dev_years = 4  # 20
test_years = 5  # 30
N_dev = 365 * dev_years
...
# weights_file = 'trained_network_weights/trained_network_weights/weights_tunedparams_leadtime1params_36500_10_1.h5'
...
for count_init, i_init in enumerate(np.random.randint(0, len(X_test), size=20)):
    ...
    clim_run['time'] = pd.date_range('20000101',
                                     pd.to_datetime('20000101') + pd.Timedelta(str(N_steps * lead_time) + 'd'),
                                     freq=str(lead_time) + 'd')
    ...
    for aggregation in (1, 10, 30):
        ...
        plt.savefig('newtork_' + str(aggregation) + 'dayvariance_leadtime' + str(lead_time) + '_init' + str(
            count_init) + '.pdf')
